...
try:
    ...
    insert_query = """
        INSERT INTO kamar (
            kd_kamar, kd_bangsal, trf_kamar,
            status, kelas, statusdata
        ) VALUES (
            %(kd_kamar)s, %(kd_bangsal)s, %(trf_kamar)s,
            %(status)s, %(kelas)s, %(statusdata)s
        )
    ...
    print(f"{target_cursor.rowcount} baris berhasil dimigrasikan ke kamar dari API.")
