...
def sync_departemen():
    ...
    try:
        ...
        source_cursor.execute("SELECT dep_id, nama FROM departemen")
        ...
        for row in rows:
            target_cursor.execute(
                "SELECT nama FROM dim_departemen WHERE dep_id = %s",
                (row['dep_id'],)
            )
            ...
            if not existing or existing['nama'] != row['nama']:
                query = """
                    INSERT INTO dim_departemen (dep_id, nama, created_at, updated_at)
                    VALUES (%s, %s, NOW(), NOW())
                    ON DUPLICATE KEY UPDATE
                        nama = VALUES(nama),
                        updated_at = NOW()
                """
                target_cursor.execute(query, (row['dep_id'], row['nama']))
                changed_rows.append(row['dep_id'])

        target_conn.commit()

        if changed_rows:
            print(f"Update: {len(changed_rows)} baris di dim_departemen.")
        else:
