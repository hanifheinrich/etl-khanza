...
def sync_suplier_medis():
    ...
    try:
        ...
        source_cursor.execute("SELECT kode_suplier, nama_suplier FROM datasuplier")
        ...
        for row in rows:
            target_cursor.execute(
                "SELECT nama_suplier FROM dim_suplier_medis WHERE kode_suplier = %s",
                (row['kode_suplier'],)
            )
        ...
        if changed_rows:
            print(f"Update: {len(changed_rows)} baris di dim_suplier_medis.")
        else:
